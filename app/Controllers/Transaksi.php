...
class Transaksi extends BaseController {
    ...
    public function index() {
        $data = [
            'pemasok' => $this->supplier_model->detailPemasok(),
        ];
    ...
    public function stok() {
        if ($this->request->isAJAX()) {
            ...
            return DataTables::use ('tb_stok')
                ->select('tb_stok.id, tb_stok.jumlah, tb_stok.keterangan, tb_stok.created_at, 
                tb_spareparts.kode_spareparts, spareparts, 
                tb_supplier.nama')
    ...
    public function tambah() {
        $uri  = $this->request->uri;
        $data = [
            'supplier' => $this->supplier_model->detailPemasok(),
        ];
        if ($uri->getSegment(2) == 'masuk') {
